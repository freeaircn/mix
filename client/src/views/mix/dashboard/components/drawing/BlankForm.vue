 ...
 * @LastEditors: freeair
 * @LastEditTime: 2023-02-21 22:00:22
-->
<template>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        dwg_name: [{ required: true, message: '请输入', trigger: ['change'] }],
        keywords: [{ required: true, message: '请输入', trigger: ['change'] }]
      },
      //
  ...
  methods: {
    ...
    handleDeleteFile (file) {
      ...
      this.fileNumber = this.fileNumber - 1
      if (file.status === 'done') {
        return apiDeleteFile({ id: file.response.id })
          .then(() => {
            return true
          })
          .catch(() => {
