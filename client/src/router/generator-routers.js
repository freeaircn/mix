...
export const buildDynamicRouter = (menus) => {
  ...
  const routers = filterDynamicRouter(menus)
  routers.push(notFoundRouter)

  return routers
}

export const filterDynamicRouter = (routers) => { // 遍历后台传来的路由字符串，转换为组件对象
  return routers.filter(router => {
    ...
    if (router.hideChildrenInMenu) {
      router.hideChildrenInMenu = router.hideChildrenInMenu === '1'
    }
    ...
    if (router.meta.hiddenHeaderContent) {
      router.meta.hiddenHeaderContent = router.meta.hiddenHeaderContent === '1'
    }
    if (router.meta.permission) {
      router.meta.permission = []
    }
    //
    if (router.component) {
      // router.component = (constantRouterComponents[router.component]) || (() => import(`@/views/${router.component}`))
      if (router.component === 'BasicLayout') { // Layout组件特殊处理
        router.component = BasicLayout
      } else if (router.component === 'BlankLayout') { // Layout组件特殊处理
        router.component = BlankLayout
      } else if (router.component === 'RouteView') { // Layout组件特殊处理
        router.component = RouteView
      } else if (router.component === 'PageView') { // Layout组件特殊处理
        router.component = PageView
      } else {
        router.component = _import(router.component)
      }
    }
    if (router.children && router.children.length) {
      router.children = filterDynamicRouter(router.children)
    }
    return true
  })
}
